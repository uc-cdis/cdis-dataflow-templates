...
from bucket_manifest.bucket import compute_md5, get_object_chunk_data
...
def test_get_object_chunk_data():
    sess = MagicMock()
    sess.request.return_value = MockedResponse(206, "example")
    assert get_object_chunk_data(sess, "bucket_name", "blob_name", 0, 10) == "example"


def test_fail_get_object_chunk_data():
    sess = MagicMock()
    sess.request.return_value = MockedResponse(404, "example")
    assert get_object_chunk_data(sess, "bucket_name", "blob_name", 0, 10) == None


@patch("bucket_manifest.bucket.get_object_chunk_data")
...
def test_get_md5_from_metadata(
    mock_get_object_md5_from_metadata, mock_get_object_size, mock_get_object_chunk_data
):
    """
    ...
    assert mock_get_object_chunk_data.called == False
...
def test_compute_md5_streaming(
    ...
    compute_md5("test", "test")
    assert mock_get_object_chunk_data.called == True
